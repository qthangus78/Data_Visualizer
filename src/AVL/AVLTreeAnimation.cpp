...
void CreateAVL::handle(){
    ...
    if(type != Type::None){
        ...
        if((key == KEY_ENTER  || (CheckCollisionPointRec(GetMousePosition(),buttonVar::buttonGo.rect) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))) && type == Type::Init) initProcess = true;
        if((key == KEY_ENTER || (CheckCollisionPointRec(GetMousePosition(),buttonVar::buttonGo.rect) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))) && type == Type::File) fileProcess = true;
    }
}
//Insert
...
void InsertAVL::InsertAnimation(int x){
    ...
    if (getBalance(prev) > 1 && (x < prev->left->data || prevType == RotateType::RightLeft) && progress == 0.0f && !mAVL->getPause()) {
        type = RotateType::RightRight;
        rightRotate(mAVL->getRoot(),prev,mAVL->getEdge());
        for(int i = path.size()-1; i>=0; i--)  path[i]->height = std::max(getHeight(path[i]->left),getHeight(path[i]->right)) + 1;
        handlePos(mAVL,oldPos,newPos);
        if(prevType == RotateType::None){
            InsertState state(prev,RotateType::RightRight);
            rotate.push_back(state);
        }
        else if(prevType == RotateType::RightLeft){
            InsertState state(prev,RotateType::RightLeft);
            rotate.push_back(state);
        }
    }  // LL case
    else if (getBalance(prev) < -1 && (x > prev->right->data || prevType == RotateType::LeftRight) && progress == 0.0f && !mAVL->getPause()) {
        type = RotateType::LeftLeft;
        leftRotate(mAVL->getRoot(),prev,mAVL->getEdge());
        for(int i = path.size()-1; i>=0; i--) path[i]->height = std::max(getHeight(path[i]->left),getHeight(path[i]->right)) + 1;
        handlePos(mAVL,oldPos,newPos);
        if(prevType == RotateType::None){
            InsertState state(prev,RotateType::LeftLeft);
            rotate.push_back(state);
        }
        else if(prevType == RotateType::LeftRight){
            InsertState state(prev,RotateType::LeftRight);
            rotate.push_back(state);
        }
    }    // RR case
...
inProcess(false), returnNode(false), DeleteNodeFlag(false), runAtOnce(false), deleteLeaf(false), firstDelete(false) {
...
void ClearAVL::handle(){
    ...
    handleButtonsClickAVL(mAVL);
    int key = GetKeyPressed();
    if(key == KEY_ENTER || (CheckCollisionPointRec(GetMousePosition(),buttonVar::buttonGo.rect) && IsMouseButtonPressed(MOUSE_BUTTON_LEFT))){
