...
graph::graph(int _numNode) {
    ...
    // Dynamically scale IdealEdgeLength based on screen size and number of nodes
...
Vector2 graph::RepulsiveForce(int u, int v) {
    ...
    float forceMagnitude = Crep / (dist * dist);
...
void graph::Draw() {
    ...
    for (int u = 1; u <= numNode; ++u) {
        DrawCircleV(Nodes[u].Pos, nodeRadius, BLUE);
        DrawText(TextFormat("%d", Nodes[u].val), Nodes[u].Pos.x - 5, Nodes[u].Pos.y - 5, 15, WHITE);
    }
}
...
graph* GenerateRandomGraph() {
    int numNode = 5 + (gen() % 10);  // Random number of nodes between 5 and 14
    ...
    for (int i = 1; i <= numNode; ++i) {
        ...
        myGraph->AddEdge(u, v, 1);
    }

    // Add random edges with probability 0.1 (sparser graph)
    std::uniform_real_distribution<float> disProb(0.0f, 1.0f);
    for (int u = 1; u <= numNode; ++u) {
        for (int v = u + 1; v <= numNode; ++v) {
            if (v == (u % numNode) + 1) continue;  // Skip cycle edges
            if (disProb(gen) < 0.1f) {  // Reduced probability for sparser graph
                myGraph->AddEdge(u, v, 1);
            }
        }
    }
